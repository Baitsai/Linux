...
離開作用域時，`Frame` 會自動銷毀。
C++11 引入的智慧指標，負責獨占管理動態配置的物件。當它離開作用域或被銷毀時，會自動釋放記憶體。其擁有權無法共享或複製，只能透過 std::move 轉移，不再使用 new/delete
...
進入 LLDB 後：(lldb) breakpoint set --name main
         ...
         => 如果顯示 Breakpoint 1: 32 locations.
            這代表 LLDB 找到 32 個名稱叫 main 的位置，通常因為正在除錯的執行檔連結了很多函式庫，其中可能包含其他符號或除錯資訊。
...
在連結階段加入 ASan runtime:編譯器雖然已經在 .cpp 中插入檢查程式碼，但最後產生執行檔時，還必須連結 ASan 的 runtime library
